<template>
    <v-main>
        ...
        <v-row v-if="authenticated">
          <template v-if="!Soundkits.length == 0">
          ...
          <template v-else>
            <v-col cols="12" md="8" class="secondary darken-2 rounded-lg text-center" offset="0" offset-md="2">
              <h1 v-resize-text="{ratio:1.3, minFontSize: '14px', maxFontSize: '30px', delay: 200}">NO SOUNDKITS ADDED</h1>
...
export default {
  ...
  created () {
    this.Soundkits = this.User.soundkits
  },
  data () {
    return {
      Soundkits: null
